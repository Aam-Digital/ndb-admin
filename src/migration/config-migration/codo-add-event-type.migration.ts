...
/* eslint-disable @typescript-eslint/no-explicit-any --
   Config JSON is deeply nested and untyped; any is pragmatic here. */
...
function addVeranstaltungenToDetailView(
  ...
  // Check if event component already exists anywhere in this view
  ...
  // Find target tab using layered anchor strategy
  const targetPanel =
...
export const codoAddEventType: MigrationDefinition = {
  ...
  async run(ctx): Promise<MigrationResult> {
    // Load config document first to fail fast if missing
    ...
    // Step 1: Add enum documents (separate from config doc)
    ...
    // Steps 2–5 operate on the config data object
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const d = data as Record<string, any>;
    let configChanged = false;

    // Step 2: Add entity:event, view:event, view:event/:id
    ...
    // Step 3: Append "event" to relatedEntities.additional in Note and Todo
    ...
    // Step 3b: Rename entity:Note relatedEntities label
    ...
    // Step 4: Append event entry to navigation menu
    if (appendEventToNavMenu(d, warnings)) {
      configChanged = true;
    }

    // Step 5: Add Veranstaltungen component to mentor/mentee detail views
    ...
    // Write config document if changed
    if (configChanged) {
