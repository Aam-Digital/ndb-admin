...
/** Minimal codo-like config without any event-related entries. */
...
describe('codoAddEventType migration', () => {
  it('should apply all changes on first run', async () => {
    ...
    // Note relatedEntities updated (string → array with event)
    expect(
    ...
    // Veranstaltungen added to mentee view
  ...
  it('should skip existing enum docs without overwriting', async () => {
    ...
    expect(result.changed).toBe(true); // Config changes still applied
  ...
  it('should skip existing entity:event without overwriting', async () => {
    ...
    expect(result.changed).toBe(true); // Other changes still applied
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const config = store['app/Config:CONFIG_ENTITY'] as any;
  ...
  it('should find tab by content-based fallback', async () => {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const data = (store['app/Config:CONFIG_ENTITY'] as any).data;
    // Rename the tab title to something unusual
    ...
    expect(result.changed).toBe(true);
    // Should still find the tab via content-based fallback
